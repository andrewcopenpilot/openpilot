...
# Service Request Message structure
#
# Service Identifier - Byte 1
# Sub-function (optional) - Byte 2
# Data (optional) - Byte 2+ or 3+ depending on if Sub-function was used

# Positive Repsonse Message structure
#
# Service Identifier/Message Number - Byte 1
# Sub-function (optional) - Byte 2
# Data (optional) - Byte 2+ or 3+ depending on if Sub-function was used
#
# Service Request Messages are wrapped in USDT

# USDT - Unacknowledged Segmented Data Transfer structure
#
# bits 0-3 Frame type (Single = 0, First = 1, Consecutive = 2, FlowControl = 3)
#
# Single - bits 4-7 - Data Length, byte 2 and 3 unused
# First - bits 4-7 - Data Length high (MSB), byte 2 - Data Length Low (LSB), Byte 3 unused
# Consecutive - bits 4-7 - Sequence Number (Rolling counter for each message sent)
# FlowControl - bits 4-7 - Flow Status (0 continue to send, 1 wait, 2 terminate), byte 2 - Block Size (Number of frames before a new flowcontrol message from the receiver is required 0 = send all frames), byte 3 - Seperation Time (Minimum time between messages in microseconds)


class USDT_FRAME_TYPE
  SINGLE = 0
  FIRST = 1
  CONSECUTIVE = 2
  FLOW_CONTROL = 3
...
def createUSDTSingleFrame(data):
  frame = bytes(8)
  dateLength = len(data)

  if (dateLength <= 7):
    frame[0] = USDT_FRAME_TYPE.SINGLE << 4 | dateLength
    return frame + data


class SERVICE_IDENTIFIER_REQUEST(IntEnum):
  CLEAR_DIAGNOSTIC_INFORMATION = 0x04
  INITIATE_DIAGNOSTIC_OPERATION = 0x10
  READ_FAILURE_RECORD_DATA = 0x12
  READ_DATA_BY_IDENTIFIER = 0x1A
  RETURN_TO_NORMAL_OPERATION = 0x20
  READ_DATA_BY_PARAMETER_IDENTIFIER = 0x22
  READ_MEMORY_BY_ADDRESS = 0x23
  SECURITY_ACCESS = 0x27
  DISABLE_NORMAL_COMMUNICATION = 0x28
  DYNAMICALLY_DEFINE_MESSAGE = 0x2C
  DEFINE_PID_BY_ADDRESS = 0x2D
  REQUEST_DOWNLOAD = 0x34
  TRANSFER_DATA = 0x36
  WRITE_DATA_BY_IDENTIFIER = 0x3B
  TESTER_PRESENT = 0x3E
  REPORT_PROGRAMMING_STATE = 0xA2
  PROGRAMMING_MODE = 0xA5
  READ_DIAGNOSTIC_INFORMATION = 0xA9 
  READ_DATA_BY_PACKET_IDENTIFIER = 0xAA
  DEVICE_CONTROL = 0xAE

class SERVICE_IDENTIFIER_RESPONSE(IntEnum):
  NEGATIVE_RESPONSE = 0x7F

  #Positive Responses
  CLEAR_DIAGNOSTIC_INFORMATION = 0x44
  INITIATE_DIAGNOSTIC_OPERATION = 0x50
  READ_DATA_BY_IDENTIFIER = 0x5A
  RETURN_TO_NORMAL_OPERATION = 0x60
  READ_DATA_BY_PARAMETER_IDENTIFIER = 0x62
  READ_MEMORY_BY_ADDRESS = 0x63
  SECURITY_ACCESS = 0x67
  DISABLE_NORMAL_COMMUNICATION = 0x68
  DYNAMICALLY_DEFINE_MESSAGE = 0x6C
  DEFINE_PID_BY_ADDRESS = 0x6D
  REQUEST_DOWNLOAD = 0x74
  TRANSFER_DATA = 0x76
  WRITE_DATA_BY_IDENTIFIER = 0x7B
  TESTER_PRESENT = 0x7E
  REPORT_PROGRAMMING_STATE = 0xE2
  PROGRAMMING_MODE = 0xE5
  READ_DIAGNOSTIC_INFORMATION = 0x80 # 0x81 and 0x82 may also be valid
  READ_DATA_BY_PACKET_IDENTIFIER = 0x00 # 01 .. 9x7F and 90 .. OxFE may also be valid 
  DEVICE_CONTROL = 0xEE
...
# Alpha Codes 2 are for firmware 2 are for part number, don't know which is which because they are all AA
# Request       0x041A DC  0x041A DB  0x041A D2  0x041A D1
# Read messages 0x045A DC  0x045A DB  0x045A D2  0x045A D1
